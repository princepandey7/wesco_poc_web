<?php
	...
	class Connect{
		...
		public function InsertOrderQuery($table, $cols, $exclude = array())
		{
		 	try
			{	
				$colCount = count($cols);
		...
		public function InsertQueryLastId($table, $cols)
		{
			try
			{	
				$colCount = count($cols);
		...
		public function UpdateOrderQuery($table, $cols, $condition)
		{
			try
			{	
				$colCount = count($cols);
		...
		public function getCatNameFromCart($productid)
		{
			try
			{
				$stmt = $this->db->prepare("select categories_name from product INNER JOIN categories ON (product.categories_id=categories.categories_id) where pro_id='{$productid}'");
				$stmt->execute();
				$results =  $stmt->fetch(PDO::FETCH_ASSOC);
		...
		public function getPageTitle($categories_name)
		{
			try
			{
				$stmt = $this->db->prepare("select * from  categories where  categories_name='{$categories_name}' ");
				$stmt->execute();
				//$results =  $stmt->fetch(PDO::FETCH_ASSOC);
			...
			public function getSubCatPageTitle($sub_categories_name)
		{
			try
			{
				$stmt = $this->db->prepare("select * from  product where  pro_name='{$sub_categories_name}' ");
				$stmt->execute();
				//$results =  $stmt->fetch(PDO::FETCH_ASSOC);
